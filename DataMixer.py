...
class SourceMixer(object):
	...
	def mix(self, x, y, shift_factor=None, shift_overlaps=False, **kwargs):
		random.seed(self.seed) #난수생성ㅈ
		np.random.seed(self.seed)
  
		ids, counts = np.unique(y, return_counts=True) # 각각의 데이터가 클래스의 요소가 / 몇개가 있는지
		...
		xy_dict = {i: [x_i for (x_i,y_i) in zip(x, y) if y_i==i] for i in ids} #class가 2개 들어가면 2개에 대해 i와 
