...
2. Generate the config.json file containing the configurations for constructing datasets, building and training models, and evaluating model performance 
   ...
   ```command
   python ConfigGenerator.py --animal Bird --file config.json
   ```
...
3. Generate the datasets for training and evaluating the classifier and separator models
   ...
   ```command
   python DataGenerator.py --animal Bird --data_directory Data --config config.json --os Windows --objective Classification --regime Closed
   ```

   ```command
   python DataGenerator.py --animal Bird --data_directory Data --config config.json --os Windows --objective Separation --regime Closed
   ```
   ...
   ```command
   python DataGenerator.py --animal Bird --data_directory Data --config config.json --os Windows --objective Separation --regime Open
   ```
4. Train the classifier model, which is used to evaluate the performance of the separator model on a downstream task

   ```command
	python Classifier.py --animal Bird --data Data --config config.json
   ```
...
5. Train the separator model including a classifier to evaluate performance as well as the classifier's testing accuracy to account for the probabilistic nature of classifying biacoustic signals and the stochasticity of the classifier model
   ...
   ```command
	python Separator.py --animal Bird --data Data --config config.json --classifier_name 1nllloss_accuracy_ --classifier_peak_acc 100.0 --regime Closed
   ```
